...
const introductionDiv = document.querySelector('#introduction'),
    ...
    completionDiv = document.querySelector('#completion'),
...
function submit(orderPick) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
            ...
            numPicks = response.Picks;
            numPicksDiv.textContent = response.Picks + ' / 100';
            picksDiv.style.display = 'inline';
            if (response.Picks == 100) {
                completionDiv.style.display = 'inline';
                return;
            }
            setsDiv.style.display = 'inline';
            if (response.Picks == 25 || response.Picks == 50 || response.Picks == 75 ||
                response.Picks == 90) {
                let msg = 'You&rsquo;ve submitted ' + response.Picks + ' responses! ';
                if (response.Picks % 90 == 0)
                    msg += 'Only ten more to go!';
...
function cycles(pick) {
    setPick = pick;
    const pickedSet = numPicks % 2 == 0 ? set1 : set2;  // Deterministically choose set for cycle question
    for (let i = 1; i <= 4; i++)
        for (let j = 0; j < 10; j++) {
